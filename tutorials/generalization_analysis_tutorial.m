...
toolbox_dir = 'C:\OneDrive\Lab\ESIN_Ephys_Files\Analysis\phyzzyML\dependencies\Neural Decoding Toolbox 1.0.4\ndt.1.0.4\';
raster_file_directory_name = 'C:\OneDrive\Lab\ESIN_Ephys_Files\Analysis\phyzzyML\dependencies\Neural Decoding Toolbox 1.0.4\Zhang_Desimone_7objects_raster_data\';
...
[the_training_label_names, the_test_label_names] = deal(cell(size(id_string_names)));
for iID = 1:length(id_string_names)   
...
for iTrainPosition = 1:length(position_names)
    
    % load the results from each training and test location
    for iTestPosition = 1:length(position_names)
